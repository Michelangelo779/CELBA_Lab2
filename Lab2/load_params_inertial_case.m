...
mld.BD = 0.0; % load viscous coeff (n.a.) % overall mech load params
mld.J = mld.JD + gbox.J; % total inertia
mld.B = 2e-6; % total viscous fric coeff (estimated)
mld.tausf = 1.0e-2; % total static friction (estimated) %% Voltage driver nominal parameters
% op−amp circuit params
drv.R1 = 7.5e3;
drv.R2 = 1.6e3;
drv.R3 = 1.2e3;
% op−amp input resistor (dac to non−inverting in)
% op−amp input resistor (non−inverting in to gnd)
% op−amp feedback resistor (output to inverting in)
drv.R4 = 0.5e3; % op−amp feedback resistor (inverting in to gnd) 
drv.C1 = 100e-9; % op−amp input capacitor
drv.outmax = 12; % op−amp max output voltage % voltage driver dc−gain
...
% [pulses] −> [deg] % [pulses] −> [rad] % [deg] −> [pulses]
...
daq.adc.bits = 16;
daq.adc.fs = 10;
daq.adc.q = 2*daq.adc.fs/(2^daq.adc.bits-1);
%   resolution (bits)
%   full scale (as set in SLDRT Analog Input block)
%   quantization